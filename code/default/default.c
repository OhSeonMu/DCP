...
//Modification By OSM
static struct list_head new_hugepage_freelist[PARTITION_NUM];
...
//Modification By OSM
static int add_new_hugepage_freelist(struct page *page) {
   size_t bucket = pfn_to_bucket(page_to_pfn(page));

   if(CACHE_SIZE <= bucket)
      list_add(&page->lru, &new_hugepage_freelist[1]);
   else
      list_add(&page->lru, &new_hugepage_freelist[0]);
...
//Modification By OSM
static int add_freed_hgepage_freelist(struct page *page) {
   size_t bucket = pfn_to_bucket(page_to_pfn(page));
...
static void build_page_list(void) {
   ...
   for(i = 0; i < PARTITION_NUM; i++)
      INIT_LIST_HEAD(&new_hugepage_freelist[i]);
   ...
   for_each_hstate(h) {
	 ...
	 //if(add_new_page(page) != 0)
         //   break;
	 if(add_new_hugepage_freelist(page) != 0)
            break;
      }
   }
...
static struct page *_minimize_conflicts(struct hstate *h, int nid, pid_t pid, struct vm_area_struct *vma, unsigned long addr)
{
   ...
   if(s) {
      ret = s->page;
      reserve_page(h, nid, pid, s, i, addr);
   }
   return ret;
}
...
//Modification By OSM
static struct page *new_dequeue_huge_page(struct hstate *h, int nid) {
   struct page *page;
   bool pin = !!(current->flags & PF_MEMALLOC_PIN);
   ...
   spin_lock(&lock);
   page = list_first_entry_or_null(&new_hugepage_freelist[0], struct page, lru);
   spin_unlock(&lock);
...
// Modification By OSM
static struct page *new_enqueue_freed_page(struct hstate *h, struct page *page) {
   int nid = page_to_nid(page);
   ...
   add_freed_hgepage_freelist(page);
   return page;
}
...
int s_migrate_pages(pid_t pid, unsigned long nr_pages, unsigned long *addresses, struct page **pages) {
   ...
   for(i = 0; i < nr_pages; i++) {
      ...
      isolate_hugetlb(page, &pagelist); // also increment page_ref_count, refcount == 3
...
int init_module(void)
{
   ...
   set_dequeue_hook(new_dequeue_huge_page);
   set_enqueue_hook(new_enqueue_freed_page);
